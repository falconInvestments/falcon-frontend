...
export class StocksComponent implements OnInit {
  ...
  columns = [
    {columnDef: 'stockName', header: 'Stock Name', sortBy: 'Sort by header', cell: (element: any) => `${element.name}`},
    {columnDef: 'symbol', header: 'Symbol', sortBy: 'Sort by symbol', cell: (element: any) => `${element.symbol}`},
    {columnDef: 'price', header: 'Price',  sortBy: 'Sort by Price', cell: (element: any) => `${element.price}`}
  ];
